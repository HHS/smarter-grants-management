...
The Next app uses the Next third party Google library to add the necessary scripts for instantiating Google Analytics (GA) on the site. The site is pointed at a single Google Tag Manager (GTM) account ID (`GOOGLE_TAG_MANAGER_ID`, see [environments.ts](../../frontend/src/constants/environments.ts)), which manages the creation of GA tags, and is wired into the app in [RootLayoutWrapper](../../frontend/src/components/core/RootLayoutWrapper.tsx).

# New Relic

New Relic is a monitoring platform, used by the Smarter Grants Management team to:
...
Integration of New Relic into the client and Node processes will be handled by the New Relic Node package. The Node library is able to generate the necessary scripts for the client code to report back to New Relic. [See RootLayoutWrapper](https://github.com/HHS/smarter-grants-management/blob/main/frontend/src/components/core/RootLayoutWrapper.tsx#L49) to see how this is being done.

#### Environment Variables

Environment variables containing the necessary New Relic configuration values (see above) for deployed environments are stored in SSM parameters and supplied to the app via ECS task definitions at the Next application's run time. (For more about the application's environments, [check documentation here](./environments.md)).
