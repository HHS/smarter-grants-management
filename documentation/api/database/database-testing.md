...
The test suite creates a new PostgreSQL database schema separate from the schema that are used by the application outside of testing (eg. for the `grantor` schema we create `test-{uuid}-grantor`. This schema persists throughout the testing session is dropped at the end of the test run. The schema is created by the `db` fixture in [conftest.py](../../../api/tests/conftest.py). The fixture also creates and returns an initialized instance of the DBClient that can be used to connect to the created schema.
