...
We store the state and nonce value in our `mgmt_login_gov_state` DB table.
...
any user in `mgmt_link_external_user` table. If not, we'll create a record + create a user.
In either case, we'll update the email from login.gov in this table.

We create a `mgmt_user_token_session` object in our database which has a `token_id`, `expires_at` timestamp, and `is_valid` boolean.
...
With this session token we generate a JWT containing the `token_id`, `mgmt_user_id` and a few miscellaneous JWT attributes.
...
## Using the token
In endpoints that support user authentication, you can pass a header like `X-MGMT-Token <your token>` in order to authenticate.

You can see which endpoints currently support this by going to our [OpenAPI docs](https://api.simpler.grants.gov/docs)
and finding which endpoints can consume the `X-MGMT-Token` header.
