...
This guide will help you set up the Grants Management API for local development on Windows using WSL (Windows Subsystem for Linux) and Docker Desktop.
...
6. Run `make start`
7. Verify with `curl http://localhost:8089/health`
...
```bash
cd /mnt/c/Users/"YourUsername"/Documents/GitHub/smarter-grants-management/api
```
...
```bash
cd ~/smarter-grants-management/api
```

**Option 3: Find your project location**
```bash
# In PowerShell, find your project:
# cd "C:\Users\YourUsername\Documents\GitHub\smarter-grants-management"
# Then in WSL, convert the path by replacing:
# C:\Users\YourUsername\... becomes /mnt/c/Users/YourUsername/...
# Example: /mnt/c/Users/YourUsername/Documents/GitHub/smarter-grants-management/api
```
...
- Build Docker images (grants-management-api)
...
- `api-grants-management-api` (the main API) - port 8089
- `grants-mgmt-db` (PostgreSQL database) - port 5439 (inside Docker, 5432)
- `s3mock` - port 9090
- `sqsmock` - port 9324
- `api-mock-oauth2-server-1` (authentication mock) - port 5008
...
curl -I http://localhost:8089/docs
```
...
This will seed the database with sample data for development
...
- **API Documentation (Swagger)**: http://localhost:8089/docs
- **Mock OAuth2 Server**: http://localhost:5008
...
docker compose logs grants-management-api

# Follow logs in real-time
docker compose logs -f grants-management-api
```
...
# Run specific tests
make test args="tests/api/users/test_user_route_login.py"
```
...
# Or use docker exec
docker exec -it grants-mgmt-db psql -d grants_management -U grants_management
```
