...
To limit the exposure/attack surface we build our base container from scratch. The base image is defined by [api/docker-python-base](../../api/docker-python-base), built and published by the [Build Python Base Image workflow](../../.github/workflows/cd-python-base-image.yml), and exposed through the [GitHub Packages interface/Docker Repository](https://github.com/HHS/smarter-grants-management/pkgs/container/python-base-image) as `ghcr.io/hhs/python-base-image`. [api/Dockerfile](../../api/Dockerfile) pins a specific tag and digest of that image on line 1.

The base image builds CPython from source on Amazon Linux 2023 and ships `uv`, so the two things `api/Dockerfile` relies on it for are the Python runtime and `/python-version.txt`.
...
The workflow run summary also prints the published tags and digest, so you can copy them from there instead of the packages page.
...
### For build
Update the `ARG UV_VERSION` line in the Dockerfile for the [base image](#docker-base-image-updates), [api/docker-python-base](../../api/docker-python-base),
to the version of UV that you want to upgrade to, then follow [Rebuilding the Base Image](#rebuilding-the-base-image).
...
# Run the API and verify it locally at http://localhost:8089/docs
make run-logs
```
...
* [APIFlask](https://apiflask.com/changelog/) - Running tests should verify everything is still connected properly, but you should also verify the OpenAPI docs are the same by running `make run-logs` and going to http://localhost:8089/docs
...
6. Ensure linters and formatters are passing by running the commands below in the `api` directory.
```
make format
make lint
```
