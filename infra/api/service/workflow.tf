...
# Also requires New Relic: when project_config.enable_newrelic is false the
# service module creates no host log forwarder, so these outputs are null and
# there is nothing to subscribe the workflow log group to.
resource "aws_lambda_permission" "allow_cloudwatch_workflow" {
  count = local.enable_workflow_service && module.project_config.enable_newrelic ? 1 : 0
