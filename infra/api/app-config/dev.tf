# api service for the dev environment (AWS account 135002447353).
#
# A few environment-specific settings are deferred for the initial
# infrastructure bring-up because they don't exist yet:
#   - HTTPS/custom domains: no ACM certificate or Route53 hosted zone yet.
#   - Notifications: no SES domain identity yet.
#   - New Relic entity GUIDs: create the dev entities, then fill these in.
module "dev_config" {
  source         = "./env-config"
  ...
  database_deletion_protection  = false # dev is expected to be torn down and rebuilt
  database_newrelic_entity_guid = ""    # Populate once the New Relic entity for the dev RDS cluster exists

  has_incident_management_service = local.has_incident_management_service
  enable_identity_provider        = local.enable_identity_provider
  enable_notifications            = false # Enable once an SES domain identity exists for dev

  service_newrelic_entity_guid      = "" # Populate once the New Relic entity for the dev primary ALB exists
  service_newrelic_mtls_entity_guid = "" # Populate once the New Relic entity for the dev mTLS ALB exists
  api_host_newrelic_entity_guid     = "" # Populate once the New Relic entity for the dev ECS service host exists
