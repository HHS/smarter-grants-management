...
data "aws_iam_policy_document" "github_assume_role" {
  statement {
    ...
    condition {
      ...
      # GitHub appends a context segment to the subject (":ref:refs/heads/main",
      # ":environment:staging", ":pull_request", ...), so the wildcard form is
      # what normally matches. The exact form is listed too so the policy still
      # works if a token is issued with no trailing segment. Both are anchored on
      # the full repository identifier, so neither can match a different repo.
      values = [
