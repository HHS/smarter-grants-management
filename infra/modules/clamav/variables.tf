...
variable "api_base_url" {
  description = "Base URL of the API the scanner posts results to, including the scheme. The scanner calls POST <api_base_url>/v1/files/<file_id>."
  type        = string
}
...
variable "enable_newrelic" {
  description = "Whether to ship the scanner and freshclam CloudWatch logs to New Relic. When false the forwarder Lambda, its IAM role, the subscription filters, and the /new-relic-license-key SSM lookup are all skipped — the lookup is a plain data source that fails an apply in an account with no New Relic set up. Mirrors modules/service."
  type        = bool
  default     = false
}

variable "newrelic_entity_guid" {
  description = "New Relic entity GUID to associate the forwarded ClamAV (scanner + freshclam) logs with. Set this to the API's New Relic entity GUID for the environment so ClamAV logs land on the same entity as the API logs. When null, logs are still shipped to the New Relic account but aren't bound to a specific entity. Only read when enable_newrelic is true."
  type        = string
  default     = null
}
