...
locals {
  ...
  # Everything in this file is gated on enable_newrelic. When false the license
  # key lookup below is skipped too, which is the point: it is a plain data
  # source and would fail an apply in an account with no New Relic set up.
  nr_count = var.enable_newrelic ? 1 : 0
}

data "aws_ssm_parameter" "newrelic_license_key" {
  count = local.nr_count
...
resource "aws_lambda_function" "nr_log_forwarder" {
  ...
  role = aws_iam_role.nr_log_forwarder[0].arn
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          ArnLike = {
            "kms:EncryptionContext:aws:logs:arn" = "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${local.nr_log_forwarder_name}"
          }
        }
      },
    ]
...
resource "aws_cloudwatch_log_group" "nr_log_forwarder" {
  ...
  kms_key_id        = aws_kms_key.nr_log_forwarder[0].arn
}
...
resource "aws_iam_role_policy_attachment" "nr_log_forwarder_basic" {
  count      = local.nr_count
  role       = aws_iam_role.nr_log_forwarder[0].name
...
resource "aws_iam_role_policy" "nr_log_forwarder_ssm" {
  ...
  role  = aws_iam_role.nr_log_forwarder[0].id
...
resource "aws_lambda_permission" "allow_cloudwatch_rds" {
  ...
  function_name = aws_lambda_function.nr_log_forwarder[0].function_name
  principal     = "logs.amazonaws.com"
  source_arn    = "${data.aws_cloudwatch_log_group.rds_postgresql[0].arn}:*"
}

resource "aws_cloudwatch_log_subscription_filter" "rds_to_newrelic" {
  count           = local.nr_count
  name            = "${var.name}-rds-to-newrelic"
  log_group_name  = data.aws_cloudwatch_log_group.rds_postgresql[0].name
  filter_pattern  = ""
  destination_arn = aws_lambda_function.nr_log_forwarder[0].arn
