...
locals {
  ...
  # ALB log forwarding needs both a load balancer to forward from and New Relic
  # to forward to. enable_newrelic false also skips the license key lookup, which
  # is a plain data source that would fail in an account with no New Relic.
  nr_alb_count = var.enable_load_balancer && var.enable_newrelic ? 1 : 0
}

data "aws_ssm_parameter" "newrelic_license_key" {
  count = local.nr_alb_count
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          ArnLike = {
            "kms:EncryptionContext:aws:logs:arn" = "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${local.nr_alb_log_forwarder_name}"
          }
        }
      },
    ]
