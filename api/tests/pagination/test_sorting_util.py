...
from tests.db_test_models.db_test_models import ExampleTable
...
def test_apply_sorting_with_mapping_nulls_last():
    ...
    validate_order_by(result, "ORDER BY dummy.example.example_id ASC NULLS LAST")
    ...
    validate_order_by(
        result,
        "ORDER BY dummy.example.description DESC NULLS LAST, dummy.example.my_count ASC NULLS LAST",
    )
    ...
    validate_order_by(
        result,
        "ORDER BY dummy.example.description ASC NULLS LAST, dummy.example.example_id DESC NULLS LAST, dummy.example.my_count DESC NULLS LAST",
    )
...
def test_apply_sorting_with_mapping_no_nulls_last():
    ...
    validate_order_by(
        result,
        "ORDER BY dummy.example.description DESC, dummy.example.my_count ASC",
    )
...
def test_apply_sorting_with_model_getattr():
    ...
    validate_order_by(
        result,
        "ORDER BY dummy.example.example_id ASC, dummy.example.my_count DESC",
    )
    assert "NULLS LAST" not in str(result.compile())
...
def test_apply_sorting_with_model_getattr_nulls_last():
    ...
    validate_order_by(result, "ORDER BY dummy.example.my_count ASC NULLS LAST")
