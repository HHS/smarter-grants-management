...
def s3_scan_user_key(db_session, enable_factory_create, internal_resource):
    ...
    # We need to fetch it fresh because the session-scoped resource is detached
    internal_resource_in_session = get_internal_resource(db_session)

    # Create user with INTERNAL_S3_SCAN privilege on internal resource
    user = setup_user_with_roles(
        db_session,
        resources=[internal_resource_in_session],
...
class TestUpdateFileScanStatus422:
    ...
    def test_file_not_at_s3_location(
        ...
        assert body["errors"][0]["type"] == "file_not_found_at_location"
