...
from tests.db.models.factories import LinkExternalUserFactory, UserApiKeyFactory, UserFactory


@pytest.fixture(autouse=True)
def invalidate_jwts(db_session):
    # As this test is for an endpoint that grabs all
    # users and then generates a JWT for each
    # it ends up scaling poorly as we add more tests
    # So, make it so it doesn't find valid JWTs from prior tests
    # which keeps the performance reasonable.
    db_session.execute(update(UserTokenSession).values(is_valid=False))
...
def test_get_local_users_200(client, db_session, enable_factory_create):
    ...
    user_with_everything = UserFactory.create()
    ...
    assert user_with_everything_resp["first_name"] is None
    assert user_with_everything_resp["last_name"] is None
    assert (
