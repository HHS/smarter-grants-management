...
def test_announcement_create_duplicate_number_of_deleted_200(
    ...
    # An announcement exists with that number, but because it is deleted, that's fine.
    AnnouncementFactory.create(
        announcement_number=announcement_request["announcement_number"], is_deleted=True
    )
