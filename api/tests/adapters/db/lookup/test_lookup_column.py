...
from tests.db.models.factories import ExampleTableFactory, FriendTableFactory
from tests.db_test_models.db_test_models import (
    ExampleTable,
    ExampleType,
    FriendTable,
    FriendType,
...
def test_lookup_column_conversion(
    ...
    raw_db_value = db_session.execute(
        text(
            f"select example_type_id from {db_schema_prefix}dummy.{ExampleTable.get_table_name()} where example_id='{example.example_id}'"  # nosec
        )
    ).scalar()
