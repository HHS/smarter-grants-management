...
from src.auth.api_key_handler import MAX_KEY_GENERATION_RETRIES, ApiKeyHandler, KeyGenerationError
...
def test_create_api_key_success(enable_factory_create, db_session: db.Session, caplog):
    ...
    with caplog.at_level("INFO"):
        api_key = ApiKeyHandler(db_session).create_api_key(
...
def test_create_api_key_max_retries_exceeded(enable_factory_create, db_session: db.Session, caplog):
    ...
    with patch("src.auth.api_key_handler.generate_api_key_id") as mock_generate:
        ...
        with (
            ...
            ApiKeyHandler(db_session).create_api_key(
                user_id=user.user_id,
                key_name="Failed Key",
            )
...
def test_create_api_key_database_rollback_on_gateway_failure(
    ...
    api_key_with_name = db_session.execute(
        select(UserApiKey).where(
            ...
            UserApiKey.key_name == "Test API Key",
        )
    ).scalar_one_or_none()
...
def test_create_api_key_multiple_keys_same_user(enable_factory_create, db_session: db.Session):
    ...
    api_key1 = ApiKeyHandler(db_session).create_api_key(
        user_id=user.user_id,
        key_name="Production Key",
    )

    api_key2 = ApiKeyHandler(db_session).create_api_key(
        user_id=user.user_id,
...
def test_delete_api_key_success(enable_factory_create, db_session: db.Session, caplog):
    ...
    with caplog.at_level("INFO"):
        ApiKeyHandler(db_session).delete_api_key(user.user_id, api_key.api_key_id)
...
def test_delete_api_key_not_found_wrong_user(enable_factory_create, db_session: db.Session):
    ...
    with pytest.raises(apiflask.exceptions.HTTPError) as exc_info:
        ApiKeyHandler(db_session).delete_api_key(user2.user_id, api_key.api_key_id)
...
def test_delete_api_key_not_found_nonexistent(enable_factory_create, db_session: db.Session):
    ...
    with pytest.raises(apiflask.exceptions.HTTPError) as exc_info:
        ApiKeyHandler(db_session).delete_api_key(user.user_id, uuid.uuid4())
...
def test_delete_api_key_multiple_keys_same_user(enable_factory_create, db_session: db.Session):
    ...
    ApiKeyHandler(db_session).delete_api_key(user.user_id, api_key1.api_key_id)
...
def test_get_user_api_keys_empty_result(enable_factory_create, db_session: db.Session):
    ...
    api_keys = ApiKeyHandler(db_session).get_user_api_keys(user.user_id)
...
def test_get_user_api_keys_single_key(enable_factory_create, db_session: db.Session):
    ...
    api_key = UserApiKeyFactory.create(key_name="Test Key")

    api_keys = ApiKeyHandler(db_session).get_user_api_keys(api_key.user_id)

    assert len(api_keys) == 1
    assert api_keys[0].api_key_id == api_key.api_key_id
    assert api_keys[0].key_name == "Test Key"
    assert api_keys[0].user_id == api_key.user_id
...
def test_get_user_api_keys_only_users_keys(enable_factory_create, db_session: db.Session):
    ...
    user1_key = UserApiKeyFactory.create(key_name="User 1 Key")
    user2_key = UserApiKeyFactory.create(key_name="User 2 Key")

    user1_keys = ApiKeyHandler(db_session).get_user_api_keys(user1_key.user_id)

    assert len(user1_keys) == 1
    assert user1_keys[0].api_key_id == user1_key.api_key_id
    assert user1_keys[0].key_name == "User 1 Key"

    user2_keys = ApiKeyHandler(db_session).get_user_api_keys(user2_key.user_id)
...
def test_get_user_api_key_success(enable_factory_create, db_session: db.Session):
    ...
    result = ApiKeyHandler(db_session).get_user_api_key(api_key.user_id, api_key.api_key_id)
...
def test_get_user_api_key_not_found_wrong_user(enable_factory_create, db_session: db.Session):
    ...
    with pytest.raises(apiflask.exceptions.HTTPError) as exc_info:
        ApiKeyHandler(db_session).get_user_api_key(user2.user_id, api_key.api_key_id)
...
def test_get_user_api_key_not_found_nonexistent(enable_factory_create, db_session: db.Session):
    ...
    with pytest.raises(apiflask.exceptions.HTTPError) as exc_info:
        ApiKeyHandler(db_session).get_user_api_key(user.user_id, uuid.uuid4())
...
def test_rename_api_key_success(enable_factory_create, db_session: db.Session, caplog):
    ...
    with caplog.at_level("INFO"):
        renamed_api_key = ApiKeyHandler(db_session).rename_api_key(
            user_id=user.user_id,
...
def test_rename_api_key_wrong_user(enable_factory_create, db_session: db.Session):
    ...
    with pytest.raises(apiflask.exceptions.HTTPError) as exc_info:
        ApiKeyHandler(db_session).rename_api_key(
...
def test_rename_api_key_same_name(enable_factory_create, db_session: db.Session):
    ...
    renamed_api_key = ApiKeyHandler(db_session).rename_api_key(
        user_id=user.user_id, api_key_id=api_key.api_key_id, key_name="Same Key Name"
    )
...
def test_rename_api_key_multiple_keys_same_user(enable_factory_create, db_session: db.Session):
    ...
    renamed_api_key = ApiKeyHandler(db_session).rename_api_key(
        user_id=user.user_id, api_key_id=api_key2.api_key_id, key_name="Renamed Key 2"
    )
...
def test_rename_api_key_long_name(enable_factory_create, db_session: db.Session):
    ...
    renamed_api_key = ApiKeyHandler(db_session).rename_api_key(
        user_id=user.user_id, api_key_id=api_key.api_key_id, key_name=long_name
    )
