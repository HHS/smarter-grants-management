...
from src.adapters.db.lookup import Lookup, LookupRegistry, LookupTable

logger = logging.getLogger(__name__)


def _sync_lookup_values(db_client: PostgresDBClient | None = None) -> None:
    """
...
def _sync_roles(db_session: db.Session) -> None:
    ...
    # We import CORE_ROLES here as the setup causes SQLAlchemy
    # to attempt to figure out its relationships between models
    # in the wrong order causing issues. If we allow that to happen
    # normally when the app starts running, then there isn't an issue.
