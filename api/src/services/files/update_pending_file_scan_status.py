...
def update_pending_file_scan_status(
    ...
    internal_resource = get_internal_resource(db_session)
    enforcer = AuthorizationEnforcer(db_session)
    enforcer.verify_access(user, {Privilege.INTERNAL_S3_SCAN}, internal_resource)
        ...
        raise_flask_error(
            ...
            validation_issues=[
                ValidationErrorDetail(
                    type="file_not_found_at_location",
                    message="File does not exist at the provided s3 path",
                    field="file_location",
                )
            ],
