...
def verify_user_can_access_workflow(
    ...
    if AuthorizationEnforcer(db_session).can_access(
        user=user,
        required_privileges={Privilege.INTERNAL_WORKFLOW_EVENT_SEND},
        resource=get_internal_resource(db_session),
    ):
        logger.info("User has internal workflow event permissions and can send events")
        return

    # Without a workflow there's nothing to approve against, which is the case for
    # start events - those stay internal-only.
    if workflow is None:
        logger.info("User does not have permissions to send a start workflow event")
        raise_flask_error(403, "Forbidden")

    if not can_user_do_approval(
        db_session, user=user, workflow=workflow, config=config, event_to_send=event_to_send
    ):
        logger.info(
...
def ingest_workflow_event(
    ...
    - For start_workflow events:
      ...
      - The resource exists (404 if not) and its type is the one the workflow expects (422 if not)
      - No active workflow of that type already exists for the resource (422 if one does)
    - For process_workflow events:
      ...
      - For approval events, the response type is present and allowed by the approval's
        configuration (422 if not)
    """
...
def _validate_start_workflow_event(
    ...
    # Validate the resource exists and is of the type this workflow attaches to. Unlike
    # find/apply there's no entity type in the request to cross-check - the type comes
    # off the resource row and is compared against the workflow's configuration.
    get_workflow_entity(
        db_session,
        resource_id=start_context.resource_id,
        config=config,
    )

    # Validate no active concurrent workflow exists for this resource
    validate_no_concurrent_workflow(
        db_session,
        resource_id=start_context.resource_id,
        config=config,
...
def _validate_process_workflow_event(
    ...
    # Note: process_workflow_context is guaranteed to be present due to schema validation
    ...
    # Get the config and state machine class for this workflow type
    ...
    verify_user_can_access_workflow(
        ...
        workflow=workflow,
        config=config,
        event_to_send=process_context.event_to_send,
    )
