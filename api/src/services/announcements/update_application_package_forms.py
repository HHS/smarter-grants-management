...
def _reconcile_forms(db_session: db.Session, request_forms: list[dict], application_package: ApplicationPackage):
    existing_form_map: dict[int, ApplicationPackageForm] = {form.form_id: form for form in application_package.application_package_forms}
    ...
    for form_data in request_forms:
        ...
        if existing_package_form:
            ...
            db_session.add(ApplicationPackageForm(
    ...
    for existing_package_form in application_package.application_package_forms:
        if existing_package_form.form_id not in target_form_ids:
            logger.info("Removing application package form", extra={"form_id": existing_package_form.form_id})
            application_package.application_package_forms.remove(existing_package_form)

def update_application_package_forms(db_session: db.Session, user: User, announcement_id: uuid.UUID, application_package_id: uuid.UUID, json_data: dict) -> ApplicationPackage:
