...
def get_application_package(
    ...
    application_package = db_session.execute(
        ...
        .options(
            ...
            selectinload(ApplicationPackage.announcement_assistance_listing).selectinload(
    ...
    if application_package is None:
        raise_flask_error(
            404, message=f"Could not find application package with ID {application_package_id}"
        )

    return application_package


def get_application_package_and_verify_access(
    db_session: db.Session,
    user: User,
    announcement_id: uuid.UUID,
    application_package_id: uuid.UUID,
) -> ApplicationPackage:
    application_package = get_application_package(
        db_session, announcement_id, application_package_id
    )
