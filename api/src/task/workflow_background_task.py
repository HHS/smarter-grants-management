...
def workflow_background_task(
    ...
    It also deliberately skips ecs_background_task's maintenance-mode check.
    That check returns early, which is right for a task that runs on a schedule
    but wrong for a long-running service: exiting would just have ECS restart
    the task in a loop for the length of the maintenance window. The caller
    idles instead.
    """
