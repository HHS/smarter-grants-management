...
class ApprovalProcessor:
    ...
    def handle_approval_accepted(self) -> WorkflowApproval:
        """Handle receiving an Approved event"""
        return self._handle_approval_event(ApprovalResponseType.APPROVED)

    def handle_approval_declined(self) -> WorkflowApproval:
        """Handle receiving a Declined event"""
        return self._handle_approval_event(ApprovalResponseType.DECLINED)

    def handle_approval_requires_modification(self) -> WorkflowApproval:
        """Handle receiving a Requires Modification event - also marks all prior events as invalid"""
        curr_approval = self._handle_approval_event(ApprovalResponseType.REQUIRES_MODIFICATION)
