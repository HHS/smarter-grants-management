...
class EventHandler:
    ...
    * Fetches the entity associated with the workflow
    ...
    * Instantiates and runs the workflow against the underlying state machine
    ...
    def _pre_process_new_workflow_event(self) -> StateMachineEvent:
        ...
        * Verifying the entity exists / makes sense for the given workflow
        * Fetching the acting user
        ...
        # Resolves the resource, and errors if its type isn't what this workflow is
        # configured for. The entity itself isn't needed to build the workflow record
        # (that only stores the resource ID), but resolving it is what proves the
