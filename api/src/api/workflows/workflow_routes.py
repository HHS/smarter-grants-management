...
@workflow_blueprint.doc(
    ...
    description="Get a workflow's current state, audit history, approvals, and approval configuration.",
...
def workflow_get(db_session: db.Session, workflow_id: uuid.UUID) -> response.ApiResponse:
    ...
    add_extra_data_to_current_request_logs({"workflow_id": workflow_id})
...
@workflow_blueprint.doc(
    ...
    description="Get the same payload as Get Workflow, resolved from one of its event IDs.",
    responses=[200, 401, 403, 404],
)
@workflow_blueprint.auth_required(jwt_or_api_user_key_multi_auth)
@flask_db.with_db_session()
def workflow_get_by_event_id(db_session: db.Session, event_id: uuid.UUID) -> response.ApiResponse:
    ...
    add_extra_data_to_current_request_logs({"event_id": event_id})
...
@workflow_blueprint.doc(
    ...
    description="Get a workflow's audit history, paginated.",
    responses=[200, 401, 403, 404, 422],
)
@workflow_blueprint.auth_required(jwt_or_api_user_key_multi_auth)
@flask_db.with_db_session()
